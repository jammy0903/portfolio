...
export const projects = [
  ...
  },
  {
    slug: "memedics",
    featured: true,
    badge: "LIVE · memedics.space 운영 중",
    title: "memedics",
    subtitle: "한국 밈 트렌드 사전 — 여러 소스에서 밈 활성도를 측정하는 SSR 서비스",
    type: "개인 프로젝트 — 아키텍처·백엔드·프론트·크롤러 1인 개발 / 운영 중",
    ...
    highlights: [
      "홈 SSR 마지막 직렬 구간 제거 — Supabase 1000행 페이지 상한 탓에 전체 조회가 순차 왕복이 됐는데, Prefer: count=exact로 총량을 먼저 묻는 방식은 meme_cards(4중 집계 조인) 뷰를 전부 구체화해 오히려 2배 느렸다(실측 1995ms → 1053ms). 총량을 묻지 않고 앞 2페이지를 무조건 병렬로 쏘는 방식으로 2왕복 → 1왕복. 이 과정에서 정렬 tiebreaker 버그도 발견 — created_at이 대량삽입 탓에 1000행에 distinct 16개뿐이라 동점 정렬이 페이지 간 중복·누락을 만들어 id를 유일 tiebreaker로 강제",
      "CORS 우회와 SSRF 방어를 동시에 만족하는 이미지 프록시(/img) — 결과카드를 canvas로 그려 toBlob으로 내보내는데 외부 CDN이 CORS 헤더를 주지 않아 캔버스가 오염되는 문제를, 서버가 받아 same-origin으로 되돌려주는 프록시로 해결. 오픈 프록시가 곧 SSRF가 되므로 허용 도메인 화이트리스트 + 원시 IP/로컬 차단 + 8MB 상한 + 매직바이트 검증을 함께 걸었다(일부 호스트가 정상 이미지를 application/octet-stream으로 내려줘 content-type만으론 판별 불가)",
      "짤 검색 하이브리드 파이프라인 — 키워드 정확일치 → caption 부분일치 → 벡터 유사도(multilingual-e5-large) 3단 폴백. 질의 임베딩은 HF 추론 API로 만들고 jjal_queries 테이블에 캐시해 미스일 때만 외부 호출. 코사인 거리 컷 0.19는 감이 아니라 실측으로 확정(관련 짤 0.14~0.18, 무관 케이스 0.199부터). 별칭 사전 108그룹으로 질의를 확장하고 확장 질의를 병렬로 발사",
      "브라우저↔DB 직결을 끊고 서버 라우트를 API 경계로 승격 — 정적 프론트가 Supabase에 직접 붙어 프론트가 DB 스키마에 결합돼 있던 구조에서, service_role 키를 서버가 쥐고 RLS를 2차 방어로 내리는 구조로 전환. 프론트는 스키마가 아니라 API 계약에만 의존",
      "빅뱅 재작성 대신 스트랭글러 마이그레이션 — 바닐라 정적 사이트 → SvelteKit SSR을 Phase 1~5로 쪼개 각 단계가 독립 배포 가능한 상태를 유지. Phase 1은 '순위 SSR 1페이지 end-to-end'라는 얇은 수직 슬라이스로 새 아키텍처 전체를 먼저 실증하고, 구 사이트는 기능 파리티 도달까지 병행 운영 후 스왑",
      "의도적 비추상화(YAGNI) — 유튜브·네이버·커뮤니티는 진짜 다형성이라 SourcePort 하나만 인터페이스로 두고, Repo 인터페이스·DI 컨테이너·전면 헥사고날은 두 번째 구현이 실제로 생길 때까지 도입하지 않음. 집계·순위는 앱으로 끌어오지 않고 SQL에 유지하고, 도메인 TS에는 판단(스테디 판정·후보 승격 조건·매칭)만 남김",
    ],
    images: [
      ...
      { src: "/images/memedics/jjal-search.jpg", caption: "짤 벡터 검색 — '퇴근'으로 검색하면 단어가 없는 '집에 가고싶다'·'너무힘들어서 죽으련다' 짤까지 의미로 찾아낸다" },
