...
export const projects = [
  ...
  },
  {
    ...
    dogGallery: [
      { src: "/images/dog-walk/breeds/cheese-rest.webp", label: "치즈 시바" },
      { src: "/images/dog-walk/breeds/corgi-rest.webp", label: "웰시코기" },
      { src: "/images/dog-walk/breeds/chihuahua-rest.webp", label: "치와와" },
      { src: "/images/dog-walk/breeds/poodle-rest.webp", label: "토이푸들" },
      { src: "/images/dog-walk/breeds/dachshund-rest.webp", label: "닥스훈트" },
      { src: "/images/dog-walk/breeds/bulldog-rest.webp", label: "프렌치불독" },
      { src: "/images/dog-walk/breeds/border-rest.webp", label: "보더콜리" },
      { src: "/images/dog-walk/breeds/chow-rest.webp", label: "차우차우" },
      { src: "/images/dog-walk/breeds/ig-rest.webp", label: "이탈리안 그레이하운드" },
      { src: "/images/dog-walk/breeds/golden-rest.webp", label: "골든리트리버" },
      { src: "/images/dog-walk/breeds/jaemi-rest.webp", label: "재미" },
    ],
    links: {
  ...
  },
  {
    ...
    highlights: [
      ...
      "**정렬 tiebreaker 버그 발견 및 수정.** created_at이 대량삽입 탓에 1000행에 distinct 16개뿐이라 동점 정렬이 페이지 간 중복·누락을 만들고 있었고, id를 유일 tiebreaker로 강제해 해결했다.",
      "**CORS 우회와 SSRF 방어를 동시에 만족하는 이미지 프록시 구현.** 캔버스 오염을 same-origin 프록시로 풀되 오픈 프록시가 되지 않게 도메인 화이트리스트·원시 IP 차단·8MB 상한·매직바이트 검증을 함께 걸었다.",
      "**짤 검색 3단 폴백 파이프라인 설계.** 키워드 정확일치 → 부분일치 → 벡터 유사도(e5-large) 순으로 떨어뜨리고, 질의 임베딩은 캐시해 미스일 때만 외부 호출한다.",
      "**벡터 컷오프 0.19를 실측으로 확정.** 관련 짤은 0.14~0.18, 무관 케이스는 0.199부터라는 분포를 직접 측정해 정했고, 별칭 사전 108그룹으로 질의를 확장해 병렬 발사한다.",
      "**브라우저↔DB 직결을 끊고 서버 라우트를 API 경계로 승격.** service_role 키를 서버가 쥐고 RLS는 2차 방어로 내려, 프론트가 DB 스키마 대신 API 계약에만 의존하게 만들었다.",
      "**빅뱅 재작성 대신 스트랭글러 마이그레이션.** 정적 사이트 → SvelteKit SSR을 Phase 1~5로 쪼개 각 단계를 독립 배포 가능하게 유지하고, 구 사이트는 파리티 도달까지 병행 운영했다.",
      "**의도적 비추상화(YAGNI).** 진짜 다형성인 SourcePort 하나만 인터페이스로 두고, 집계·순위는 SQL에 남긴 채 도메인 TS에는 판단 로직만 담았다.",
    ],
    images: [
      { src: "/images/memedics/home.jpg", caption: "memedics 홈. SSR로 렌더되는 밈 사전 첫 화면" },
      { src: "/images/memedics/ranking.jpg", caption: "스테디 순위표. 각 밈마다 '3~4개 소스 측정' 활성도 기반 랭킹" },
